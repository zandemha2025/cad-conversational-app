...
import {
  ...
  Scan, Link2, Gauge, FileText, Microscope, Ruler, History,
} from 'lucide-react';
...
function WorkspaceInner({ projectId }: { projectId: string | undefined }) {
  ...
  const leftButtons: {
    ...
    { id: 'history',           icon: <History size={15} />,      title: 'Version History' },
  ];
  ...
  function getPanelColor(id: LeftPanel, active: boolean) {
    ...
    switch (id) {
      ...
      case 'history':           return 'bg-indigo-700 text-white';
          ...
          <>
            {showLeft && (
              <div className="w-full md:w-72 shrink-0 border-r border-cadsurface-700 overflow-hidden flex flex-col md:flex">
                ...
                {leftPanel === 'history'           && <VersionHistoryPanel projectId={projectId} />}
              </div>
            )}
